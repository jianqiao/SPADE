...
public class CDM extends AbstractReporter{
			...
			try{
				while(!dataReaders.isEmpty()){
					...
					while((tccdmDatum = (TCCDMDatum)dataReader.read()) != null){
						...
						processDatum(tccdmDatum.getDatum());
					}
					try{
						dataReader.close();
					}catch(Exception e){
	...
	public boolean launch(String arguments){
		...
		if(CommonFunctions.isNullOrEmpty(inputFileArgument)){
			...
			if(configMap == null || configMap.isEmpty()){
				...
				dbg_size = Integer.parseInt(configMap.get("dbg_trigger_size"));
				dbg_millis = Long.parseLong(configMap.get("dbg_sleep_millis_on_trigger"));
	...
	private int dbg_size;
	private long dbg_millis;
	...
	private void printStats(){
		...
		if(internalBufferSize > dbg_size){logger.log(Level.SEVERE, "Going to sleep until buffer emptied enough");}
		boolean slept = false;
		while(getBuffer().size() > dbg_size){
			slept = true;
			try{Thread.sleep(dbg_millis);}catch(Exception e){}
		}
		if(slept){
			logger.log(Level.SEVERE, "WOKE UP FROM SLEEP!");
		}
	}
		...
		if(datum != null){
			...
			if(datumClass.equals(UnitDependency.class)){
				...
				if(uuid != null && vertex != null){
					...
					logger.log(Level.SEVERE, "uuid={0}, vertex={1}, datum={2}, datumClass={3}", new Object[]{uuid, vertex, datum, datum.getClass()});
				}
			}
		}		
	}
